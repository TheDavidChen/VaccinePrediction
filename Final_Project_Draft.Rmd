...
Since we are dealing with 2 classification problems, we explored the classification methods from the class. Specifically, we looked at regression methods, tree-based methods and support vector machines. We compared the test set results of all these methods and found that some of them worked well while others did not. We found that logistic regression, generalized boosted modeling and random forest methods performed the best overall. For seasonal vaccination prediction, they had overall test set accuracies of 77.8%, 77.8%, and 77.9% respectively. For h1n1 vaccination prediction, they had overall test set accuracies of 83.1%, 83%, and 83% respectively.
...
After removing the missing data, we can split the dataset into training and test sets. We will usee this training set to train all our models and evaluate their performances on the test set. We do a 60-40 stratified split such that 60% of the overall data are in the training set while also making sure that each of the training and test set have equal representation of our 2 response variables.
...
mean(model_test$seasonal_vaccine== test.pred)
...
seasonalflu_results_table <- results_table %>% mutate("Overall Accuracy"=c(0.778,0.764,0.758,0.72,0.732,0.77,0.777,0.778,0,0,0),"Sensitivity"=c(0.743,0.637,0.617,0.739,0.687,0.748,0.754,0.743,0,0,0),"Specificity"=c(0.81,0.881,0.888,0.702,0.773,0.791,0.799,0.811,0,0,0))

kable(seasonalflu_results_table %>% arrange(desc(`Overall Accuracy`)))
```

From the table above, we can see that full logistic regression and generalized boosted models that we created were the most accurate overall. The generalized boosted model may have a slight edge here since it has a slightly higher specificity. We should note that the performances of the full logistic regression, generalized boosted model and the random forest model were comparable with each other. The other methods did not seem to work well with this specific classification problem. 
...
h1n1flu_results_table <- results_table %>% mutate("Overall Accuracy"=c(0.831,0.821,0.817,0.78,0.814,0.825,0.83,0.83,0,0,0),"Sensitivity"=c(0.478,0.335,0.303,0.614,0.373,0.474,0.457,0.474,0,0,0),"Specificity"=c(0.936,0.966,0.97,0.829,0.946,0.93,0.941,0.936,0,0,0))

kable(h1n1flu_results_table %>% arrange(desc(`Overall Accuracy`)))
```

From this table, we can see that once again full logistic regression, random forest and generalized boosted models gave the best results. Even though logistic regression's performance was a little better overall, the other 2 models had comparable performances. 

Since these 3 methods performed well for our classification problems, if we were to proceed forward with further investigations for the competition, we should stick with one of these methods.
