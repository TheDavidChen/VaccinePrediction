...
It looks like an accuracy of 73.1% is better than chance. However, an error rate of 26.9% is still not great. We can look into ways to prune this tree to see if they can lead to improved performance.
...
Once again, we get an accuracy of 0.731. This suggests that pruning the above tree doesn't really affect the performance and they lead to the exact same predictions. Therefore, let's just look at the test set performance of the full tree.
...
The tree has test set accuracy of 73.6% (test error rate of 26.4%).
...
We can see that bagging results in a out of bag (OOB) error rate of just 22.9% (accuracy of 77.1%), which is much better than 26.9% training error rate we got for the pruned and unpruned trees. This suggests that this model might perform better on test set.
...
The bagging model has a test set accuracy of 77.7% (test error rate of 22.3%).
...
Here, we can see that using random forest with 500 trees and 5 random features considered at each split results in an out of bag (OOB) error rate of 22.2% (an accuracy of 77.8%) which is slightly better than the error rate obtained using the bagging method.
...
The random forest model had a test set accuracy of 78.5% (a test error rate of 21.5%).
...
Our cross-validation suggested that the most optimal number of trees to use for this data would be 3548. 
...
```{r, warning=FALSE}
boost.pred.test=predict(boost_seasonal,seasonal_test,type="response",n.trees=3548)
...
Here, we see that our boosted model with 3548 trees resulted in a test set accuracy of 78.4% (test error rate of 21.6%).
...
It looks like we get an accuracy of 81.6% (an error rate of 18.4%), which is pretty good. Let's see if pruning this tree can lead to improved performance.
...
Once again, we get an accuracy of 81.6%. This suggests that pruning the above tree doesn't really affect the performance and they lead to the exact same predictions. Therefore, let's just look at the test set performance of the full tree.
...
The tree has test set accuracy of 81.2% (test error rate of 18.8%).
...
We can see that bagging results in a out of bag (OOB) error rate of just 17.5% (accuracy of 82.5%), which is much better than 18.8% training error rate we got for the pruned and unpruned trees. This suggests that this model might perform better on test set.
...
Here, we can see that using random forest with 500 trees and 5 random features considered at each split results in an out of bag (OOB) error rate of 16.9% (an accuracy of 83.1%) which is slightly better than the error rate obtained using the bagging method.
...
The random forest model had a test set accuracy of 82.8% (a test error rate of 17.2%).
...
Our cross-validation suggested that the most optimal number of trees to use for this data would be 3959. 
...
```{r, warning=FALSE}
boost.pred.test=predict(boost_h1n1,h1n1_test,type="response",n.trees=3959)
...
Here, we see that our boosted model with 3548 trees resulted in a test set accuracy of 83.1% (test error rate of 16.9%).
